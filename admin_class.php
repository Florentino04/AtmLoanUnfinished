			...
			if(isset($status)){
				...
				if($status == 2){
					...
					for($i= 1; $i <= $plan['months'];$i++){
						...
						$dates = date("Y-m-d",strtotime($date."+15 days"));
					$chk = $this->db->query("SELECT * FROM loan_schedules where loan_id = $id and date(date_due) ='$date'  ");
					if($chk->num_rows > 0){
						$akinse = date("Y-m-d",strtotime($cutoff."+ 14 days"));
						$atrenta = date("Y-m-d",strtotime($date."+ 1 day"));
						$ls_id = $chk->fetch_array()['id'];
						//$this->db->query("UPDATE loan_schedules set loan_id = $id, date_due ='$atrenta' where id = $ls_id ");
						//$this->db->query("UPDATE loan_schedules set loan_id = $id, date_due ='$akinse' where id = $ls_id ");
					}else{
						$akinse = date("Y-m-d",strtotime($date."- 14 days"));
						$atrenta = date("Y-m-d",strtotime($date."+ 1 day"));
						$this->db->query("INSERT INTO loan_schedules set loan_id = $id, date_due ='$akinse' ");
						$this->db->query("INSERT INTO loan_schedules set loan_id = $id, date_due ='$atrenta' ");
